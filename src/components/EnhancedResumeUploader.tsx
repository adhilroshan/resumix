...
const getFileTypeIcon = (fileType: string, size: number = 24) => {
  switch (fileType) {
    case 'application/pdf':
      return <IconFileTypePdf size={rem(size)} stroke={1.5} />;
    case 'text/plain':
      return <IconFileText size={rem(size)} stroke={1.5} />;
    case 'text/x-tex':
    case 'application/x-tex':
      return <IconFileTypeDoc size={rem(size)} stroke={1.5} />;
    default:
      return <IconFile size={rem(size)} stroke={1.5} />;
  }
};

interface EnhancedResumeUploaderProps {
  onResumeProcessed?: (text: string, fileName: string) => void;
  isLoading?: boolean;
...
export function EnhancedResumeUploader({ 
  ...
  // Process the file without verification
  const processSimple = (file: FileWithPath) => {
    if (file.type === 'application/pdf') {
      if (onResumeProcessed) {
        // PDF content will be extracted by parent component
        onResumeProcessed('', file.name);
      }
    }
  };
  ...
  const parseLatexFile = async (file: FileWithPath) => {
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        if (onResumeProcessed) {
          onResumeProcessed(cleanedText, file.name);
        }
      };
  ...
  return (
    <Paper p={isSmall ? "sm" : "md"} withBorder radius="md">
      <Stack gap={isSmall ? "xs" : "md"}>
        ...
        <Alert 
          icon={<IconInfoCircle size={isSmall ? 14 : 16} stroke={1.5} />} 
          variant="light"
          color="blue"
          p={isSmall ? "xs" : "sm"}
        >
        ...
        {error && (
          <Alert 
            icon={<IconAlertCircle size={isSmall ? 14 : 16} stroke={1.5} />} 
            color="red" 
            title="Upload Error" 
            p={isSmall ? "xs" : "md"}
          >
        ...
        {!isProcessing && !success ? (
          <>
            ...
            >
              <Group justify="center" gap={isSmall ? "sm" : "xl"} style={{ minHeight: rem(isSmall ? 100 : 120), pointerEvents: 'none' }}>
                <Dropzone.Accept>
                  <IconUpload size={rem(isSmall ? 30 : 36)} stroke={1.5} />
                </Dropzone.Accept>
                <Dropzone.Reject>
                  <IconX size={rem(isSmall ? 30 : 36)} stroke={1.5} />
                </Dropzone.Reject>
                <Dropzone.Idle>
                  {file ? (
                    <Stack gap={isSmall ? "8px" : "xs"} align="center">
                      {getFileTypeIcon(file.type, isSmall ? 20 : 24)}
            ...
            <Group justify="center">
              <Button 
                ...
                leftSection={<IconUpload size={isSmall ? 14 : 16} stroke={1.5} />}
                disabled={isLoading}
                size={isSmall ? "sm" : "md"}
                fullWidth={isSmall}
            ...
            {file && file.type === 'application/pdf' && showVerification && (
              <Group justify="center" mt={isSmall ? "xs" : "sm"}>
                <Button 
                  color="blue"
                  onClick={processPDF}
                  leftSection={<IconFileTypePdf size={isSmall ? 14 : 16} stroke={1.5} />}
                  size={isSmall ? "sm" : "md"}
                  fullWidth={isSmall}
                >
                  Process PDF
          ...
          <Box>
            <Alert 
              ...
              icon={<IconCheck size={isSmall ? 16 : 18} stroke={1.5} />} 
              mb={isSmall ? "xs" : "md"}
            >
                ...
                {extractedData.contactInfo && (extractedData.contactInfo.name || extractedData.contactInfo.email || extractedData.contactInfo.phone) && (
                  <Grid.Col span={12}>
                    <Box mb={isSmall ? "xs" : "sm"}>
                      ...
                      <List spacing="xs" size={isSmall ? "xs" : "sm"}>
                        {extractedData.contactInfo.name && (
                          <List.Item
                            icon={
                              <ThemeIcon color="blue" variant="light" size={isSmall ? 18 : 22}>
                                <IconUser size={isSmall ? 12 : 14} stroke={1.5} />
                              </ThemeIcon>
                            }
                          >
                            {extractedData.contactInfo.name}
                        ...
                        {extractedData.contactInfo.email && (
                          <List.Item
                            icon={
                              <ThemeIcon color="blue" variant="light" size={isSmall ? 18 : 22}>
                                <IconMail size={isSmall ? 12 : 14} stroke={1.5} />
                              </ThemeIcon>
                            }
                          >
                            {extractedData.contactInfo.email}
                        ...
                        {extractedData.contactInfo.phone && (
                          <List.Item
                            icon={
                              <ThemeIcon color="blue" variant="light" size={isSmall ? 18 : 22}>
                                <IconPhone size={isSmall ? 12 : 14} stroke={1.5} />
                              </ThemeIcon>
                            }
                          >
                            {extractedData.contactInfo.phone}
